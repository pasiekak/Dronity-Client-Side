...
const Activation = () => {
    ...
    useEffect(() => {
        ...
        ServerCommunicator.handleRequest('get', `/activation?token=${queryParams.get('token')}`).then(res => {
            if(res.success) {
                setSuccess(true);
            }
            setShow(true);
    ...
    return (
        <div className='activation'>
            ...
            {show && <>
                {success ?
                <p>
                    Pomyślnie zweryfikowano użytkownika i utworzono konto. Możesz teraz zalogować się na nie zalogować.
                </p>:
                <p>
                    Konto zostało już zweryfikowane. Spróbuj się zalogować.
                </p>}
            </>}
            <Links toLogin />
        </div>
